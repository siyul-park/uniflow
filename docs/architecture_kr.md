...
노드 명세는 각 노드의 역할을 선언적으로 정의하며, 이러한 명세들이 서로 연결되어 워크플로우를 형성합니다. 각 워크플로우는 특정 네임스페이스에 정의되며, 각 런타임 환경은 하나의 네임스페이스를 실행합니다. 네임스페이스는 다른 네임스페이스에 정의된 노드를 참조할 수 없으며, 각각 격리되어 관리됩니다.
