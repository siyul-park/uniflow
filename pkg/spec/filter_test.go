...
func TestFilterHelper(t *testing.T) {
	...
	var testCase = []struct {
		...
		{
			when:   Where[uuid.UUID](KeyID).Equal(id),
			expect: &Filter{OP: database.EQ, Key: KeyID, Value: id},
		},
		{
			when:   Where[uuid.UUID](KeyID).NotEqual(id),
			expect: &Filter{OP: database.NE, Key: KeyID, Value: id},
		},
		{
			when:   Where[uuid.UUID](KeyID).LessThan(id),
			expect: &Filter{OP: database.LT, Key: KeyID, Value: id},
		},
		{
			when:   Where[uuid.UUID](KeyID).LessThanOrEqual(id),
