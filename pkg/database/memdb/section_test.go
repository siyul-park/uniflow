...
func TestSection_AddConstraint(t *testing.T) {
	...
	c := Constraint{
		...
		Keys:    []string{"_id"},
		Unique:  true,
		Partial: func(_ types.Map) bool { return true },
	}
...
func TestSection_Scan(t *testing.T) {
	t.Run("Flat", func(t *testing.T) {
		t.Run("FastPath", func(t *testing.T) {
			...
			count := 0
			child.Range(func(d types.Map) bool {
				assert.Equal(t, doc, d)
				count += 1
				return true
			})
	...
	t.Run("Deep", func(t *testing.T) {
		t.Run("FastPath", func(t *testing.T) {
			...
			keyDepth1 := types.NewString(faker.UUIDHyphenated())
			keyDepth2 := types.NewString(faker.UUIDHyphenated())
			...
			count := 0
			child2.Range(func(d types.Map) bool {
				assert.Equal(t, doc, d)
				count += 1
				return true
			})
